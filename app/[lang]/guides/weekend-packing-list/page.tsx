...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    ...
    alternates: { canonical: `/${lang}/guides/weekend-packing-list`, languages: { "nb-NO": "/no/guides/weekend-packing-list", "en-GB": "/en/guides/weekend-packing-list", "x-default": "/no/guides/weekend-packing-list" } },
  };
}
...
export default async function WeekendPackingList({ params }: PageProps) {
  ...
  const jsonLd = { "@context": "https://schema.org", "@graph": [
    { "@type": "Article", headline: no ? "Pakkeliste for weekendtur" : "Weekend trip packing list", inLanguage: no ? "nb-NO" : "en-GB", publisher: { "@type": "Organization", name: "Flyferie.no" } },
  ...
  return <main className="min-h-screen bg-[#fffaf1] text-[#17332f]">
      <GuideBreadcrumbSchema lang={lang === "no" ? "no" : "en"} slug="weekend-packing-list" />
    ...
    <section className="bg-[#f4d7a1] px-5 py-12 lg:px-8"><div className="mx-auto flex max-w-7xl flex-col gap-6 rounded-[28px] bg-white/70 p-7 sm:p-10 lg:flex-row lg:items-center lg:justify-between"><div><p className="text-xs font-bold uppercase tracking-[.2em] text-[#b94f3d]">{no ? "Pakk enda lettere" : "Pack even lighter"}</p><h2 className="display mt-3 text-[34px] font-bold leading-tight sm:text-4xl">{no ? "Kan hele turen tas med håndbagasje?" : "Can the entire trip fit in cabin baggage?"}</h2></div><Link href={`/${lang}/guides/cabin-bag-weekend`} className="inline-flex w-fit shrink-0 rounded-full bg-[#17332f] px-6 py-3.5 font-bold text-white">{no ? "Les håndbagasjeguiden" : "Read the cabin baggage guide"} →</Link></div></section>
  </main>;
}
