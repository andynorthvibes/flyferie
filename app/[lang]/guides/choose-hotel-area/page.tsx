...
export default async function ChooseHotelAreaPage({ params }: PageProps) {
  ...
  return <main className="min-h-screen bg-[#fffaf1] text-[#17332f]">
    ...
    <section className="px-5 py-12 sm:py-16 lg:px-8"><div className="mx-auto max-w-4xl"><p className="text-xs font-bold uppercase tracking-[.2em] text-[#e16f59]">{norwegian ? "Vanlige spørsmål" : "Frequently asked questions"}</p><div className="mt-7 space-y-4">{faq.map((item) => <article key={item[1]} className="rounded-[22px] border border-[#17332f]/10 bg-white p-6 shadow-sm"><h2 className="text-xl font-bold">{norwegian ? item[0] : item[1]}</h2><p className="mt-3 leading-7 text-[#48645f]">{norwegian ? item[2] : item[3]}</p></article>)}</div><div className="mt-8 flex flex-wrap gap-3"><Link href={`/${lang}/destinations`} className="inline-flex rounded-full bg-[#17332f] px-6 py-3.5 font-bold text-white">{norwegian ? "Finn reisemål og bydeler" : "Explore destinations and areas"} →</Link><Link href={`/${lang}/guides/compare-hotel-prices`} className="inline-flex rounded-full border border-[#17332f]/25 bg-white px-6 py-3.5 font-bold">{norwegian ? "Sammenlign hotellpriser riktig" : "Compare hotel prices properly"} →</Link></div></div></section>
  </main>;
}
