    ...
    <main className="min-h-screen bg-[#fffaf1] text-[#17332f]">
      <header className="bg-[#102f2b] text-white">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-5 lg:px-8">
          <Link href={`/${lang}`} aria-label="Flyferie.no – forsiden">
            <Image src="/flyferie-logo-v9.png" alt="Flyferie.no" width={480} height={200} priority className="h-auto w-[190px] sm:w-[225px]" />
          </Link>
      ...
      <section className="bg-[#f4d7a1] px-5 py-14 lg:px-8 lg:py-20">
        <div className="mx-auto max-w-7xl">
          ...
          <div className="mt-9 grid gap-5 sm:grid-cols-2 lg:max-w-[820px]">
  ...
  return (
    <Link href={`/${lang}/destinations/${place.slug}`} className="group relative min-h-[330px] overflow-hidden rounded-[26px] bg-[#17332f] text-white shadow-sm transition hover:-translate-y-1 hover:shadow-xl">
      ...
      <div className="absolute bottom-0 left-0 right-0 p-6">
        <p className="text-sm text-white/70">{lang === "no" ? place.countryNo : place.countryEn}</p>
        <h3 className="display mt-1 text-3xl font-bold">{name}</h3>
        <p className="mt-3 text-sm font-bold">{lang === "no" ? "Les reiseguiden" : "Read the travel guide"} →</p>
      </div>
    </Link>
  );
}
